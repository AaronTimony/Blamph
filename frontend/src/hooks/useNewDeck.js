...
const combineDecksWithPercentages = (decks, percentages) => {
  const percentMap = new Map(
    percentages.map(deck => [deck.deck_name, deck.known_per])
  )

  return decks.map(deck => ({
    ...deck,
    known_percentage: percentMap.get(deck.deck_name || 0).toFixed(1)
  }))
}
...
export function useDecks() {
  ...
  const availableDecksQuery = useQuery({
    ...
    queryFn: async () => {
      const [allDecks, ownedDecks, knownPercentages] = await Promise.all([
...
export function useMyDecks() {
  ...
  const searchDecksQuery = useQuery({
    queryKey: ['myDecks', 'search', user?.id],
    queryFn: () => deckQueries.searchMyDecks(apiCall, debouncedSearchQuery),
    enabled: debouncedSearchQuery.length > 0
  })

  return {getMyDecks, deleteDeck, setSearchQuery, searchQuery, searchDecksQuery}
}
