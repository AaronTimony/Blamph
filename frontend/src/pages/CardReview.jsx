...
function CardReview() {
  ...
  const get_new_card = async () => {
    try{
      const response = await authorizedFetch("http://127.0.0.1:8000/api/v1/review/newcards")
  ...
  const get_due_card = async () => {
    try{
      const response = await authorizedFetch("http://127.0.0.1:8000/api/v1/review/reviewcards")
  ...
  useEffect(() => {
    ...
    const send_rating = async () => {
      if (isCardNew) {
        try{
          const response = await authorizedFetch("http://127.0.0.1:8000/api/v1/review/newcardrating", {
            method: "POST",
        ...
        try{
          const response = await authorizedFetch("http://127.0.0.1:8000/api/v1/review/reviewcardrating", {
            method: "PATCH",
            body: JSON.stringify({"jp_word" : japWord, "rating": rating})
          })
