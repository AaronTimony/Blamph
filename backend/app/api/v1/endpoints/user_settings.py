...
from app.schemas.settings import UpdateNewWordsRequest, UpdateNewWordOrder
...
def update_user_new_word_order(request: UpdateNewWordOrder,
                                current_user: User = Depends(get_current_active_user), 
                                db: Session = Depends(get_db)):
    return settings_service.update_new_word_ordering(request.newWordOrdering, current_user.username, db)
